...
Returns all bank payment vouchers associated with a given applicant DNI. The DNI is extracted from bytes 8–15 (positions 8 to 8+8) of the `num_doc` field in the payments table using `SUBSTRING(num_doc, 8, 8)`, which encodes additional bank metadata in the leading characters.
