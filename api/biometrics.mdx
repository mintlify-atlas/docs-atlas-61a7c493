...
Stores biometric images under process directory `11` (Juli campus).
...
Stores biometric images under process directory `12` (Azangaro campus).
...
Bulk upload endpoint that accepts a facial photo together with left-hand and right-hand fingerprint images in a single request. Files are organised into separate `fotos/` and `huellas/` subdirectories. This endpoint also updates the applicant's progress record (`avance_postulante`) — setting `avance = 2` for the `inscripcion` stage and `avance = 5` for the biometric control stage.
...
<ParamField body="etapa" type="string" required>
  Stage identifier. Use `"inscripcion"` or `"control_biometrico"`. The value is lower-cased before use.
</ParamField>
...
Returns the biometric control status for an applicant identified by their code (DNI or `codigo_ingreso`). The query filters against a fixed set of active process IDs.
...
<ParamField path="codigo" type="string" required>
  The applicant's DNI or admission code (`codigo_ingreso`).
</ParamField>
...
<Tip>
  Use `GET /api/get-postulante-biometrico/{codigo}` at the biometric station before capturing images to confirm the applicant's record exists in the system and determine their current control status before proceeding with the capture workflow.
</Tip>
