...
The API also exposes a dedicated authentication group for the mobile application at `/api/app/login` and `/api/app/registro`. These endpoints follow the same Sanctum token flow but validate against `email` + `password` and return a slightly different response shape.
