...
Processes on the results page are grouped by year for easy navigation. The controller passes `procesos` to the Vue page as an array grouped by `anio`:
...
Only processes with `estado = 1` and `nivel = 1` are included in the applicant-facing view.
