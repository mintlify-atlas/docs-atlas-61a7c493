...
<Steps>
  <Step title="Paso 1 — Personal Data">
    ...
    | Field | Rule |
    |---|---|
    | `nro_doc` | Required, 8 digits for DNI |
    ...
    | `sexo` | Required (`1` = Masculine, `2` = Feminine) |
    | `estado_civil` | Optional (mapped: 1 Soltero, 2 Casado, 3 Viudo, 4 Divorciado, 5 Conviviente) |
  ...
  <Step title="Paso 2 — Contact Information">
    ...
    | `celular` | Required, min 9 digits, unique |
    | `direccion` | Required, max 255 chars |
    | `ubigeo_residencia` | Required, 6-char ubigeo code |
  </Step>
  ...
  <Step title="Paso 4 — Guardian (Apoderado) Data">
    ...
    | Field | Rule |
    |---|---|
    | `nro_documento` | Required, 8–12 chars |
    | `paterno` | Required, max 100 chars |
    | `nombres` | Required, max 200 chars |
    | `tipo_apoderado` | Required, integer in `[1, 2, 3]` |
  </Step>
...
const searchDni = async () => {
  ...
  try {
    const res = await axios.get(`/postulante/api/reniec/${form.nro_doc}`);
    if (res.data.success && res.data.mayor_edad) {
...
The `misDatos` route checks the applicant's current `avance` and automatically redirects to the first incomplete step:
...
GET /postulante/mis-datos
  ...
  → avance 4: redirect to /postulante/paso-5
  → avance ≥ 5: render MisDatos (read-only view)
```
...
<CardGroup cols={2}>
  ...
  <Card title="GET /postulante/api/colegios/{ubigeo}" icon="school">
    Retrieve the list of schools (`id`, `nombre`) for a given 6-digit ubigeo code.
  </Card>
</CardGroup>
